...
public class KeyValueList<E> {
   ...
   */
	...
	public String toString(String left, String sep, String right) {
		return (left + toString(sep, first) + right);
	}
		
		private String toString(String sep, KeyValueList<E> value) {
			if (value.next == null)
			{
				return "(" + value.key + ", " + value.elem + ")";
			}
			return "(" + value.key + ", " + value.elem + ")" + sep + toString(sep, value.next);
		}
