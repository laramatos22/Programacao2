...
public class Ngrams {
  ...
  public static String[] allNgrams(String alpha, int n) {
    ...
    if (n > 0)
	{
		String[] arr2 = allNgrams(alpha, (n-1)); //recorrencia
		arr = new String[l*arr2.length]; //atribuição de tamanho ao array
		
		int c = 0;
		for (int i = 0; i < arr2.length; i++) //enqto i for menor que o array novo
		{
			for (int k = 0; k < l; k++) //e k menor que comprimento da string
			{
				arr[c] = arr2[i] + alpha.charAt(k); //no array fica palavra do array novo mais um caracter
				c++; //incremneta c para armzenar
			}
		}
	}
