/*uma rede social caracterizada pelo conjunto
dos seus membros e pelo conjunto de pedidos de amizade já registados*/
...
/*addFriendshipRequest(m1, m2) que, dados os nomes de dois membros da rede,
regista um pedido de amizade que o primeiro envia ao segundo. Pressupõe-se
que os dois membros existem e que não existe ainda nenhum pedido entre eles.
Para este efeito, inclua em local apropriado a criação do vector requests, que
vai conter todos os pedidos de amizade, e tenha em conta que esse vector pode
encher. A dimensão inicial do vector deverá ser maxSize. Sempre que o vector
encher, deve ser expandido com mais sizeExpansion posições.*/
  /**
  ...
  public void addFriendshipRequest(String m1, String m2) {
	...
	numRequests++;
		...
		private void extendRequestArray() {
			...
			FriendshipRequest[] arrayPedidos = new FriendshipRequest[requests.length + sizeExpansion];
			
			assert arrayPedidos.length > requests.length;
			
			System.arraycopy(requests, 0, arrayPedidos, 0, numRequests);
			
			requests = arrayPedidos;
		}
  ...
  public int maxNumRequests() {
    //...
	return requests.length; //comprimento total do array requests
  }
