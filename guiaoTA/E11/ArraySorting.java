/*O programa ArraySorting.java contém a implementação de vários algoritmos de or-
denação de vectores bem como uma função main() que aplica esses algoritmos a vectores
com conteúdos gerados aleatoriamente (pela função randomArray()). Os algoritmos de
ordenação têm já algumas asserções que permitem verificar pré- e pós-condições. Se execu-
tar o programa com verificação de asserções (opção -ea), a conclusão normal da execução
indicará que os algoritmos conseguiram ordenar correctamente os vectores gerados para o
efeito. Reveja a implementação dos algoritmos e corrija quaisquer erros que encontre.*/
...
public class ArraySorting
{
  static public void main(String[] args)
  {
    if (args.length != 1) {
      ...
      err.println("Usage: java -ea ArraySorting <N>");
      exit(1);
    }
    ...
    out.println("Sequential Sort OK!");
    a = randomArray(n, n/2);
    bubbleSort(a, 0, a.length);
    out.println(Arrays.toString(a));
    out.println("Bubble Sort OK!");
    a = randomArray(n, n/2);
    insertionSort(a, 0, a.length);
    out.println(Arrays.toString(a));
    out.println("Insertion Sort OK!");
    a = randomArray(n, n/2);
    mergeSort(a, 0, a.length);
    out.println(Arrays.toString(a));
    out.println("Merge Sort OK!");
   ...
   * 
   * Para uma demonstração visual do algoritmo, ver:
  ...
  static void mergeSort(int[] a, int start, int end) {
    ...
    {        
      int mid = (start + end)/2;     // acha o meio do array;
      mergeSort(a, start, mid);  // ordena a primeira metade;
      mergeSort(a, mid, end);  // ordena a segunda metade;
  ...
  static void merge(int[] a, int start, int mid, int end) {
    ...
    while (i1 < mid) //enquanto houver elementos na parte 1
	{
		tmp[k++] = a[i1++];
	}
	while (i2 < end) // enquanto houver elementos na parte 2
	{
		tmp[k++] = a[i2++];
	}
  ...
  static void swap(int[] a, int i, int j) {
	 assert a != null;
	 assert 0 <= i && i < a.length : "i = " + i;
	 assert 0 <= j && j < a.length : "j = " + j;
